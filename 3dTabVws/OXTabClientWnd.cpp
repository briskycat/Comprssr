									 ...
									 BOOL bUpdateWindowsInfo/*=FALSE*/)
{
	...
	int nInsertIndex=m_arrTab.GetSize();
...
//	::GetClassInfoEx(AfxGetInstanceHandle(),sWndClass,&classInfo);
	...
	for(int nTabItemIndex=m_arrTab.GetSize()-1; nTabItemIndex>nIndex; nTabItemIndex--)
	{
...
int COXTabWorkspace::AddTabItemIcon(HICON hIcon)
{
	...
	// image list
	for(int nImageIndex=0; nImageIndex<m_arrImage.GetSize(); nImageIndex++)
	{
